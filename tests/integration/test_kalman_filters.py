...
def test_kf_matches_tfp_on_lgssm(lgssm_3d, sim_data_3d, tfp_ref_3d):
    ...
    tf.debugging.assert_near(m_filt, m_tfp, atol=1e-3, rtol=1e-3)
    tf.debugging.assert_near(P_filt, P_tfp, atol=1e-3, rtol=1e-3)
...
def test_kf_joseph_matches_naive(lgssm_3d, sim_data_3d):
    ...
    tf.debugging.assert_near(out_j["m_filt"], out_n["m_filt"], atol=1e-3, rtol=1e-3)
    tf.debugging.assert_near(out_j["P_filt"], out_n["P_filt"], atol=1e-3, rtol=1e-3)
