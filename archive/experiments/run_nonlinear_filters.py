...
def main():
    ...
    parser.add_argument("--tfp_pf", action="store_true", help="include TFP particle filter")
    ...
    if run_sv_flag:
        ...
        if args.tfp_pf:
            sv_cfg.filters = tuple(list(sv_cfg.filters) + ["tfp_pf"])
    ...
    if run_rb_flag:
        ...
        if args.tfp_pf:
            rb_cfg.filters = tuple(list(rb_cfg.filters) + ["tfp_pf"])
