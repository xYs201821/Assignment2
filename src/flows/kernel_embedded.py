...
class KernelParticleFlow(FlowBase):
    ...
    def _prior_from_sample(self, mu_tilde, w, batch_shape, state_dim):
        return self._sample_mean_and_cov(mu_tilde, w)
    ...
    def _scalar_kernel_and_grad(self, x, band_inv):
        x = tf.convert_to_tensor(x, dtype=tf.float32)
        band_inv = tf.convert_to_tensor(band_inv, dtype=tf.float32)
        xj = x[..., :, tf.newaxis, :]
        xk = x[..., tf.newaxis, :, :]
        diff = xj - xk  # [..., n, n, dx]
        diff_scaled = tf.einsum("...ij,...mnj->...mni", band_inv, diff)
    ...
    def _kernel_and_grad(self, x, diagB, alpha):
        ...
        xk = x[..., tf.newaxis, :, :]
        diff = xj - xk # [..., n, n, dx]
        diag_shape = tf.shape(diagB)
        dx = diag_shape[-1]

        alpha_rank = tf.rank(alpha)
        diag_rank = tf.rank(diagB)
        alpha_vec = tf.reshape(alpha, [-1])

        def _alpha_scalar():
            return tf.ones_like(diagB) * alpha

        def _alpha_rank1():
            alpha_len = tf.shape(alpha_vec)[0]
            return tf.cond(
                tf.equal(alpha_len, dx),
                lambda: tf.ones_like(diagB) * alpha_vec[tf.newaxis, :],
                lambda: tf.ones_like(diagB) * alpha_vec[..., tf.newaxis],
            )

        def _alpha_rankN():
            return tf.cond(
                tf.equal(alpha_rank, diag_rank - 1),
                lambda: tf.ones_like(diagB) * alpha[..., tf.newaxis],
                lambda: tf.broadcast_to(alpha, diag_shape),
            )

        alpha = tf.cond(
            tf.equal(alpha_rank, 0),
            _alpha_scalar,
            lambda: tf.cond(tf.equal(alpha_rank, 1), _alpha_rank1, _alpha_rankN),
        )
    ...
    def _step_control(
        ...
        if flow_tol is None:
            ...
            stop_now = flow_norm < flow_tol
            stopped = tf.logical_or(stopped, stop_now)
            ds_eff = tf.where(stopped, tf.zeros_like(ds), ds)
        return ds_eff, stopped
    ...
    def _pff_update(self, x_prior, y, w, x_mean=None, B=None):
        ...
        if self._median_alpha:
            centered = x - x_mean[..., tf.newaxis, :]
            log_n = tf.math.log(tf.cast(num_particles, tf.float32))
            log_n = tf.maximum(log_n, tf.cast(self.jitter, tf.float32))
            if self.kernel_type == "scalar":
                dist = tf.norm(centered, axis=-1)
                med = tfp.stats.percentile(dist, 50.0, axis=-1, interpolation="midpoint")
                alpha = tf.square(med) / log_n
            else:
                med = tfp.stats.percentile(
                    tf.abs(centered),
                    50.0,
                    axis=-2,
        ...
        alpha_mat = alpha[..., tf.newaxis, tf.newaxis]
        if self.kernel_type == "scalar":
            state_dim = tf.shape(x)[-1]
            eye = tf.eye(state_dim, batch_shape=tf.shape(B)[:-2], dtype=tf.float32)
            aB = alpha_mat * B + tf.cast(self.jitter, tf.float32) * eye
        ...
        for i in range(self.num_lambda):
            ...
            if self.kernel_type == "scalar":
                K, grad_K = self._scalar_kernel_and_grad(x, band_inv)
            ...
            flow = tf.reduce_sum(weighted, axis=-3)
            flow = tf.einsum("...ij,...nj->...ni", B, flow)
            ...
            ds_eff, stopped = self._step_control(
                ds,
                flow_norm,
                stopped,
                flow_tol,
            )
            x = x + ds_eff[..., tf.newaxis, tf.newaxis] * flow
        return x
    ...
    def sample(self, x_prev, y_t, w=None, seed=None):
        ...
        m0, P0 = self._prior_from_sample(mu_tilde, w, batch_shape, state_dim)
