...
class MultiTargetAcousticSSM(SSM):
    ...
    def __init__(
        ...
        sensor_xy=None,  # [Ns, 2]
        area_size=40.0,
        grid_size=5,  # default 5x5 -> 25 sensors
        dt=1.0,
        Psi=10.0,
        d0=0.1,
        cov_eps_x=None,  # [4,4] per target
        sigma_w=0.1,  # sqrt(0.01) in paper
        m0=None,  # [4C]
        P0=None,  # [4C,4C]
        seed=None,
    ):
        ...
        # Sensors: default grid in [0, area_size]x[0, area_size]
        ...
        # Single-target CV transition matrix
        dt = self.dt
        F1 = tf.stack(
            [
        ...
        # Process covariance per target
        if cov_eps_x is None:
            q = 1.0 / 20.0
            Q = q * tf.constant(
                [
                    [1.0 / 3.0, 0.0, -0.05, 0.0],
                    [0.0, 1.0 / 3.0, 0.0, -0.05],
                    [-0.05, 0.0, 1.0, 0.0],
                    [0.0, -0.05, 0.0, 1.0],
                ],
                dtype=tf.float32,
        ...
        # Block-diagonal F, Q for all targets
        ...
        # Cholesky factors for distributions
    ...
    def h(self, x):
        ...
        pos = x_reshaped[..., :, 0:2]  # [..., C, 2]

        sensor = self.sensor_xy[tf.newaxis, ...]  # [1, Ns, 2]
        pos_e = pos[..., :, tf.newaxis, :]  # [..., C, 1, 2]

        diff = pos_e - sensor  # [..., C, Ns, 2]
        dist2 = tf.reduce_sum(diff * diff, axis=-1)  # [..., C, Ns]
        contrib = self.Psi / (dist2 + self.d0)  # [..., C, Ns]
        zbar = tf.reduce_sum(contrib, axis=-2)  # sum over C -> [..., Ns]
        return zbar
